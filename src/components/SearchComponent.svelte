<script>
  import { search_store, keyboard_store, voice_store } from '../store.js';
  let search;
  let keyboard;
  ...
  function onSubmitSearch() {
    if(search) {
      window.location = `https://www.google.com.br/search?&q=${search}`;
    }
  }
</script>
...
<template>
  <section data-testid="app-search" class="App-search">
    <label class="label-search">
      Pesquise ou digite uma URL
      <svg class="icon-search" focusable="false" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"></path></svg>
      <input class="App-search-input" type="text" placeholder="Pesquise no Google ou digite um URL" on:input={e => search_store.set(e.target.value)} value={search}>
    </label>
    <button type="button" class='btn clean'  class:active='{search}' on:click={() => search_store.set('')}>
    ...
    <button type="button" class="btn voice" on:click={() => voice_store.set(true)}>
    ...
    <button type="button" class="btn search" on:click={onSubmitSearch}>
      Pesquisa Google
    </button>
